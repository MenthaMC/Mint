...
/*
 * authored by: Bacteriawa <A3167717663@hotmail.com>
...
public class ServerI18nUtil {
    ...
    public static void init() {
        ...
        logger.info("Starting load language: {}", LanguageConfig.language);

        if (preloadTask == null) {
            preloadTask = CompletableFuture.runAsync(() -> preLoadI18n(2));
        }
        
        preloadTask.thenAcceptAsync(v -> loadI18n(LanguageConfig.language, 2));
    }
    ...
    private static void preLoadI18n(int retryTime) {
        try {
            ...
            logger.warn("Unsupported language: {}", LanguageConfig.language);
        } catch (Exception e) {
